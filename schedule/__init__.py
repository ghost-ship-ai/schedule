...
class Scheduler:
    ...
    async def async_run_pending(self) -> None:
        ...
        This method can handle both sync and async jobs in the same scheduler.
        Async jobs will be awaited, while sync jobs will be executed normally.
        """
        with self._lock:
            runnable_jobs = (job for job in self.jobs if job.should_run)
            for job in sorted(runnable_jobs):
    ...
    async def _async_run_job(self, job: "Job") -> None:
        """
        Execute a job asynchronously, handling both sync and async job functions.
        """
        ret = job.run()
...
class AsyncScheduler(Scheduler):
    """
    An async-first scheduler that provides native async support for job execution.

    This scheduler is designed for use in async environments and provides
    async versions of the main scheduler methods.
    """
    ...
    async def run_all(self, delay_seconds: int = 0) -> None:
        ...
        A delay of `delay_seconds` seconds is added between each job. This helps
        distribute system load generated by the jobs more evenly over time.
    ...
    async def run_continuously(self, interval: int = 1) -> None:
        """
        Continuously run, executing scheduled jobs at intervals.

        :param interval: Time to wait between checks for scheduled jobs
        """
        while True:
...
class Job:
    ...
    * a quantity of `time units` defined by `interval`
    ...
    def __init__(self, interval: int, scheduler: Optional[Scheduler] = None):
        ...
        # async support - will be set when job_func is assigned
    ...
    def do(self, job_func: Callable, *args, **kwargs):
        ...
        # Detect if the job function is async
    ...
    def run(self):
        ...
        if self.is_async:
            # For async functions, return the coroutine object without awaiting
            ret = self.job_func()
        else:
