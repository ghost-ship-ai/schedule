...
class Scheduler:
    ...
    async def async_run_all(self, delay_seconds: int = 0) -> None:
        """
        Run all jobs regardless if they are scheduled to run or not, with async support.
...
class AsyncScheduler(Scheduler):
    """
    An async-first scheduler that provides native async support for all operations.

    This scheduler is designed for async environments and provides async versions
    of all scheduling operations. It can handle both sync and async jobs seamlessly.
    """
