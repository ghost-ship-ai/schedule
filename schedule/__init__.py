...
class Scheduler:
    ...
    async def async_run_pending(self) -> None:
        ...
        awaited, while sync jobs will be executed normally. Mixed sync/async
        jobs are supported in the same scheduler.
    ...
    async def _async_run_job(self, job: "Job") -> None:
        """
        Run a job asynchronously, handling both sync and async job functions.

        For async jobs, the coroutine is awaited. For sync jobs, they are
        executed normally. CancelJob return values are handled for both types.
        """
        ret = job.run()
...
class AsyncScheduler(Scheduler):
    ...
    async def run_pending(self) -> None:
        ...
        This is an alias for async_run_pending() to provide a consistent
        async interface.
        """
    ...
    async def run_all(self, delay_seconds: int = 0) -> None:
        ...
        A delay of `delay_seconds` is added between each job. This helps
...
class Job:
    ...
    * a quantity of `time units` defined by `interval`
    ...
    def do(self, job_func: Callable, *args, **kwargs):
        ...
        # Detect if the original function is a coroutine function
    ...
    def run(self):
        ...
        if self.is_async:
            # For async functions, return the coroutine object without calling it
            ret = self.job_func()
        else:
            # For sync functions, call and get the result as before
            ret = self.job_func()
